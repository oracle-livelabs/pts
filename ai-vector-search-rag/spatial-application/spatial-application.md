...
5. Create a table for the object shapes.
    ...
    "COMPLETED" NUMBER NOT NULL ENABLE,
...
8. Create a trigger to send deleted shapes to the audit table.
    ...
    ( ID, JSON_SHAPE, CREATED, CREATED_BY, UPDATED, UPDATED_BY, DELETED, DELETED_BY )
    VALUES
    ( :old.ID, :old.JSON_SHAPE, :old.CREATED, :old.CREATED_BY, :old.UPDATED, :old.UPDATED_BY, sysdate, v_userid );
    END;
    /
    </copy>
    ```
